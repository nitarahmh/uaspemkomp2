...
function Board (options){
  ...
  function render(){
    ...
    function htmlQ1(){
      ...
      ${buttonHTML(1, "1player", "Human Vs Computer")}
    ...
    function htmlQ3(){
      return `<div id="view1"><h3>Fill Player's Name!\n</h3>
      <form id="frm1" action="/action_page.php">
        Player1's name: <input type="text" name="playername1"><br>
        Player2's name: <input type="text" name="playername2"><br><br>
        ${buttonHTML(4, "fillname", "Submit")}
      </form>
      </div>`
    }
    function myFunction() {
    ...
    function htmlGameEnd (){
      ...
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
        ...
        let htmlAfter = `<h4>Score: ${htmlSpaces(1)} Player 1 - ${state.players[0].score} ${htmlSpaces(2)} ${state.players[1].isComputer? "Computer" : "Player 2" } - ${state.players[1].score}</h4>
        <div id='view1'><h5>${buttonHTML(3, "reload_game", "Start Game from Beginning")}</h5> </div>`
      return `<div id='resultView'> ${htmlBefore} <div id="board">${board}</id> ${htmlAfter} </div>`
    }

    let html = ''
    if (state.view == VIEW.question1) {html = htmlQ1()}
    else if (state.view == VIEW.question4) {html = htmlQ3()}
  ...
  function question1Handler (ev){
    ...
    state.view = VIEW.question4
    render()
  }
  function question4Handler (ev){
    let player1Name = $(ev.currentTarget).attr('data')
    state.players[0].symbol=player1Name;
    state.players[1].symbol=(player1Name===SYMBOLS.n1)? SYMBOLS.n1: SYMBOLS.n1;
  ...
  $(options.el).on('click', '.btnGroup3', question3Handler)
  $(options.el).on('click', '.btnGroup4', question4Handler)
